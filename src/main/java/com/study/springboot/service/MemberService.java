...
public boolean modify(MemberSaveRequestDto dto){
    ...
    public boolean sendEmail(final String getEmail){
        try {
            ...
            "하단 비밀번호 변경하기를 눌러주세요."+
            "<br><br>"+
            "<a href=\"http://15.164.134.104:8080/find/password2?getEmail="+getEmail+"\" rel=\"noopener\" style=\"display:inline-block;width:300px;height:50px;line-height:52px;background:#333;box-sizing:border-box;color:#fff;font-size:16px;font-weight:bold;text-align:center;text-decoration:none\" target=\"_blank\" >비밀번호 변경 하기</a>"+
            "<br><br>"+
            "</p>"+
            "</div>"+
            "</div>"+
